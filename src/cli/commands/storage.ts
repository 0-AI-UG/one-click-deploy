...
type Grant = { id: string; app: string; bucket: string; prefix: string; methods: string[]; createdAt: string };

export async function storage(args: string[]): Promise<void> {
  if (!args[0] || args[0] === "list") {
    const grants = await get<Grant[]>("/api/admin/storage-grants");
    table(["ID", "APP", "BUCKET", "PREFIX", "METHODS"], grants.map(grant => [grant.id, grant.app, grant.bucket, grant.prefix, grant.methods.join(",")]));
    return;
  }
  ...
  if (args[0] === "grant") {
    ...
    const grant = await post<Grant & { token: string }>("/api/admin/storage-grants", {
      app: args[1], bucket: args[2], prefix: option("prefix") ?? "",
