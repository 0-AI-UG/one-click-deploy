...
import {
  ...
  RotateCcw, Trash2, ExternalLink, Globe, Plus,
} from "lucide-react";
...
type LaidOutEdge = {
  ...
  // For env→app/service links (detachable):
...
function buildEdges(data: GraphData): LaidOutEdge[] {
  ...
  // server↔app via replicas, deduped
  ...
  // server↔service via instances, deduped
...
function edgePath(from: { x: number; y: number; w: number; h: number }, to: { x: number; y: number; w: number; h: number }) {
  // Connect from right-mid of `from` to left-mid of `to` (or reverse if to is to the left).
...
export function GraphPage() {
  ...
  // drag node / pan / link state — refs to avoid stale closures
  const dragState = useRef<
    ...
    | { mode: "link"; envId: number; cur: { x: number; y: number } }
    | null
  >(null);
  ...
  // Size the viewBox to the container.
  useEffect(() => {
  ...
  // --- env link knob ---
  const onLinkKnobDown = (e: React.MouseEvent, envId: number) => {
    if (e.button !== 0) return;
    e.stopPropagation();
    const p = toSvg(e.clientX, e.clientY);
    dragState.current = { mode: "link", envId, cur: p };
    setLinkCursor(p);
  };
  ...
  const onSvgMouseMove = (e: React.MouseEvent) => {
    ...
    if (ds.mode === "node") {
      ...
      // Convert pixel delta → viewBox delta using current scale.
      const svg = svgRef.current;
      if (!svg) return;
  ...
  const onSvgMouseUp = async (e: React.MouseEvent) => {
    ...
    if (ds.mode === "node") {
      ...
      if (target) {
        ...
        if (node && node.kind === "app") {
          await attachEnvToApp(ds.envId, node.id as number);
        } else if (node && node.kind === "service") {
          await injectEnvIntoService(ds.envId, node.id as number);
        }
      }
    }
  };
  ...
  const onNodeMouseEnter = (key: NodeKey) => {
    ...
    if (dragState.current?.mode === "link") {
      ...
      if (n && (n.kind === "app" || n.kind === "service")) setLinkHover(key);
    }
  };
  ...
  // Compute "related" set for hover-dim effect
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 animate-fade-in">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          ...
          <Btn onClick={resetLayout} variant="ghost">Reset Layout</Btn>
        </div>
      </div>

      {isEmpty ? (
        <EmptyState message="Nothing deployed yet. Deploy your first app or service to see the topology." icon={Box} />
      ) : (
        <div
        ...
        >
          ...
          >
            ...
            <g>
              {edges.map((edge) => {
                ...
                const stroke = "#111";
                const dash = edge.variant === "dashed" ? "6 4" : edge.variant === "dotted" ? "2 3" : undefined;
                // midpoint for detach button
                const mx = (a.x + a.w / 2 + b.x + b.w / 2) / 2;
                const my = (a.y + a.h / 2 + b.y + b.h / 2) / 2;
                return (
                  <g key={edge.key} opacity={dim ? 0.15 : 1}>
                    <path d={d} fill="none" stroke={stroke} strokeWidth={1.5} strokeDasharray={dash} />
              ...
              {dragState.current?.mode === "link" && linkCursor && (() => {
                const env = nodeMap.get(nk("env", dragState.current.envId));
                if (!env) return null;
                const sx = env.x; // env nodes' knob is on left edge
                const sy = env.y + env.h / 2;
            ...
            <g>
              {nodes.map((n) => {
                ...
                const fill = n.kind === "env" ? "#fffbeb" : n.kind === "volume" ? "#eff6ff" : n.kind === "server" ? "#f5f3ff" : "#ffffff";
                return (
                  <g
                    key={n.key}
                  ...
                  >
                    ...
                    <rect
                      ...
                      fill={fill}
                      stroke={isLinkTarget ? "#22c55e" : isSelected ? "#3b82f6" : "#111"}
                      strokeWidth={isLinkTarget || isSelected ? 3 : 2}
                    />
                    {/* status dot */}
                    {n.status && (
                    ...
                    {n.kind === "env" && (
                      <g
                        ...
                        onMouseDown={(e) => onLinkKnobDown(e, n.id as number)}
                      >
                        <circle cx={-2} cy={n.h / 2} r={9} fill="#fef3c7" stroke="#111" strokeWidth={2} />
                        <line x1={-6} y1={n.h / 2} x2={2} y2={n.h / 2} stroke="#111" strokeWidth={1.5} />
                        <line x1={-2} y1={n.h / 2 - 4} x2={-2} y2={n.h / 2 + 4} stroke="#111" strokeWidth={1.5} />
                      </g>
                    )}
                  </g>
                );
          ...
          {/* Floating popover for selected node */}
          {selectedNode && data && (
            <NodePopover
              node={selectedNode}
              data={data}
          ...
          <div className="absolute bottom-2 right-2 bg-white border-2 border-fg p-2 font-mono text-[8px] uppercase space-y-1 shadow-neo">
            ...
            <div className="text-[7px] text-muted normal-case pt-1 border-t border-fg/10">drag env <Plus size={8} className="inline" /> → app/service to link</div>
          </div>
        </div>
      )}
    </div>
...
function NodePopover({
  node, data, viewBox, svgRef, actionLoading, onClose, onAppAction, onSvcAction,
}: {
  node: LaidOutNode;
  data: GraphData;
      ...
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="font-mono text-[8px] text-muted uppercase">{node.kind}</div>
          </div>
        </div>
        <button
          onClick={onClose}
          className="font-mono text-[10px] text-muted hover:text-fg px-1"
          aria-label="Close"
        >×</button>
      </div>
        ...
        return (
          <div className="space-y-2">
            <div className="font-mono text-[9px] text-muted space-y-0.5">
              ...
              <div className="text-[8px] pt-1">drag the <span className="font-bold">+</span> knob onto an app or service to link</div>
            </div>
