...
export function DeployPage({ initialRepo }: { initialRepo?: string } = {}) {
  ...
  function applyManifest(idx: number, result: AppIntrospect) {
    ...
    setForm((f) => {
      ...
      return {
        ...
        // Manifest declares intent only; the env is auto-created server-side.
        webhook_staging: m.webhook?.staging ?? f.webhook_staging,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const body: DeployBody = {
      ...
      webhook_staging: form.webhook_enabled && form.webhook_staging ? true : undefined,
