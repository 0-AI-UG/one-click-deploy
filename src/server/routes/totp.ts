...
export async function handleTotpConfirmFromLogin(request: Request): Promise<Response> {
  ...
  try {
    ...
    const token = await createToken({ userId: freshUser.id, username: freshUser.username, v: freshUser.token_version });
    const permissions = freshUser.is_admin ? db.ALL_PERMISSIONS.slice() : db.getUserPermissions(userId);

    return Response.json(
      { token, user: { id: freshUser.id, username: freshUser.username, isAdmin: freshUser.is_admin === 1, totpEnabled: true, webauthnEnabled: freshUser.webauthn_enabled === 1, permissions }, backupCodes },
      { headers: corsHeaders },
    );
  } catch (error) {
    return handleError(error);
